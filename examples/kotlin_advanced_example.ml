...
let example_nested_rules () =
  ...
  print_endline
    "Find println() calls only inside classes annotated with @Service\n";
...
|}
  ...
  Printf.printf
    "Found %d methods inside classes (standalone function excluded):\n\n"
    (List.length results);
  List.iter
    (fun (r : Match.nested_match_result) ->
      let class_name =
...
let example_partial_matching () =
  ...
  print_endline
    "Find function calls that include 'timeout' as a named argument\n";
...
let example_partial_nested () =
  ...
  print_endline
    "Find classes that have methods containing 'return' statements\n";
...
let example_deep_nesting () =
  ...
  print_endline
    "Find database.query() calls inside methods inside Repository classes\n";
...
|}
  ...
  Printf.printf "Found %d database queries in Repository class methods:\n\n"
    (List.length results);
  Printf.printf
    "(Standalone query excluded because it's not inside a class)\n\n";
  List.iter
    (fun (r : Match.nested_match_result) ->
      let repo = List.assoc "$REPO" (List.nth r.contexts 0).context_bindings in
      let method_name =
        List.assoc "$METHOD" (List.nth r.contexts 1).context_bindings
      in
