...
let match_pattern =
  let doc =
    "Pattern file for matching. Uses @@ delimiters with metavar declarations. \
     See README for pattern format."
  in
  Arg.(
    value & opt (some file) None & info [ "match"; "m" ] ~docv:"PATTERN" ~doc)
...
let apply_flag =
  let doc =
    "Enable transform mode: apply semantic patch and output unified diff."
  in
    ...
    Array.fold_left
      (fun acc entry ->
        ...
        if Sys.is_directory path then begin
          ...
          if glob_match pattern path then path :: acc else acc
        end)
      acc entries
  in
...
let scan_directory ~ctx ~language ~pattern_text ~include_pattern ~exclude_dirs
    dir_path =
  ...
  List.iter
    (fun file_path ->
      ...
      | Some msg -> errors := (file_path, msg) :: !errors
      | None ->
          (* Track files with parse errors *)
          if result.parse_errors > 0 then
            files_with_parse_errors :=
              (file_path, result.parse_errors) :: !files_with_parse_errors;
          if result.matches <> [] then begin
            ...
            List.iter
              (fun m ->
                print_string
                  (format_file_match ~file_path ~source_text:result.source_text
                     m);
                print_newline ())
              result.matches
          end)
...
let scan_directory_transform ~ctx ~language ~pattern_text ~include_pattern
    ...
    (fun file_path ->
      try
        ...
        if result.edits <> [] then begin
          ...
          let diff =
            Diffract.Match.generate_diff ~file_path
...
let run file1 language list_languages match_pattern include_pattern
    exclude_patterns apply in_place =
  ...
  if list_languages then begin
  ...
  else if in_place && not apply then `Error (true, "--in-place requires --apply")
  else
    let exclude_dirs =
    ...
    | Some pattern_path, Some source_path -> (
        try
          ...
          if apply then begin
            ...
            if Sys.is_directory source_path then begin
              ...
              | None ->
                  `Error
                    ( true,
                      "Directory scanning requires --include pattern (e.g., \
                       --include '*.ts')" )
              | Some glob ->
                  scan_directory_transform ~ctx ~language ~pattern_text
                    ~include_pattern:glob ~exclude_dirs ~in_place source_path;
                  `Ok ()
            end
            else begin
              let changed, diff =
                transform_file ~ctx ~language ~pattern_text ~in_place
                  source_path
              in
          ...
          else if Sys.is_directory source_path then begin
                ...
                `Error
                  ( true,
                    "Directory scanning requires --include pattern (e.g., \
                     --include '*.ts')" )
            | Some glob ->
                scan_directory ~ctx ~language ~pattern_text
                  ~include_pattern:glob ~exclude_dirs source_path;
                `Ok ()
          end
          else begin
    ...
    | None, Some path -> (
        ...
        | Failure msg -> `Error (false, msg))
    (* No arguments *)
    | None, None -> `Error (true, "Missing required argument FILE")
...
let run_term =
  let file1 =
    Arg.(
      ...
      & info [] ~docv:"FILE" ~doc:"Source file or directory to process.")
  in
  Term.(
    ret
      (const run $ file1 $ language $ list_languages $ match_pattern
...
let cmd =
  ...
  let man =
    [
      ...
      `Pre
        "  $(tname) --apply --in-place --match patch.txt --include '*.ts' src/";
      `S "PATTERN FORMAT";
      ...
      `Pre
        ...
        \  OBJ.METHOD()";
      `P
        "Metavariable names start with a dollar sign. Types: $(b,single) (one \
         node) or $(b,sequence) (zero or more). See README for full \
         documentation.";
      `S "GLOB PATTERNS";
