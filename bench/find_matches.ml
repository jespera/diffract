(** Benchmark for Stmatch.find_matches.
    ...
    Generates synthetic TypeScript sources of varying sizes and times
    how long find_matches takes against several representative patterns.
...
let gen_source n =
  ...
  for i = 0 to n - 1 do
    ...
    if i mod 5 = 0 then
      Buffer.add_string buf (Printf.sprintf "  foo(%d);\n" i);
...
let rec count_nodes (n : _ Tree.t) =
  ...
  else 1 + List.fold_left (fun acc (c : _ Tree.child) -> acc + count_nodes c.node) 0 n.children
...
let bench_pattern ~ctx ~name ~pattern ~source =
  ...
  let results, match_ms =
    time_one (fun () -> M.find_matches pattern cursor)
  in
  Printf.printf "  %-30s | %7d nodes | %6d B | %8.2f ms | %5d matches\n"
    name nodes bytes match_ms (List.length results)
...
let () =
  ...
  List.iter
    (fun n ->
      ...
      bench_pattern ~ctx ~name:"common-first: bar($x)"
        ~pattern:pattern_common ~source;
      bench_pattern ~ctx ~name:"wildcard-first: $fn($x)"
        ~pattern:pattern_wildcard_first ~source;
      bench_pattern ~ctx ~name:"long: baz($x,$y)" ~pattern:pattern_long
        ~source;
