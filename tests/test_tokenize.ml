(** Tokenizer tests: pattern body text -> Stmatch.pattern_token list.
    ...
    Covers sigil-free metavar detection (a leaf is a metavar iff its text
    equals a declared name), concrete-token classification with observed
    node types, extras filtering, and a round-trip through the matcher
    against real source. *)
...
let test_call_with_sigil_metavar () =
  ...
  Alcotest.(check (list string))
    "foo($x)" [ "C(foo:identifier)"; "C((:()"; "S($x)"; "C():))" ]
    (show toks)
...
let test_call_with_sigil_free_metavar () =
  ...
  Alcotest.(check (list string))
    "foo(obj)" [ "C(foo:identifier)"; "C((:()"; "S(obj)"; "C():))" ]
    (show toks)
...
let test_sequence_metavar () =
  ...
  in
  Alcotest.(check (list string))
    "foo($args)" [ "C(foo:identifier)"; "C((:()"; "Q($args)"; "C():))" ]
    (show toks)
...
let test_comment_is_filtered () =
  ...
  in
  Alcotest.(check (list string))
    "comment skipped" [ "C(foo:identifier)"; "C((:()"; "S($x)"; "C():))" ]
    (show toks)
...
let test_multiple_ellipsis () =
  let toks = tokenize ~language:"typescript" ~single:[ "$x" ] "f(..., $x, ...)" in
  Alcotest.(check (list string))
    "f(..., $x, ...)"
    [
      "C(f:identifier)";
...
let test_spread_preserved () =
  let toks = tokenize ~language:"typescript" ~sequence:[ "$args" ] "f(...$args)" in
...
let tests =
  ...
  [
    ...
    test_case "round-trip binding via byte_range" `Quick
      test_roundtrip_binding;
