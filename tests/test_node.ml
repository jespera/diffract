...
let test_find_children () =
  ...
  Alcotest.(check bool) "contains function_declaration"
    true (List.mem "function_declaration" child_types)
...
let test_find_by_type () =
  ...
  Alcotest.(check bool) "found multiple identifiers"
    true (List.length identifiers >= 4)
...
let test_named_vs_all_children () =
  ...
  Alcotest.(check bool) "all_count >= named_count"
    true (all_count >= named_count)
