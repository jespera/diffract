(** Change-summary tuning parameters, gathered in one place.
    ...
    {e same} hole-fraction threshold appeared in five separate spots).
    Collecting them here gives each knob one documented home and a single source
    of truth.
...
type options = {
  ...
  max_hole_fraction : float;
      (** coherence cut (§2.3 "Coherent", §4.1): a candidate pattern's holes, as
          a fraction of its shell size, must stay below this. Also the
  ...
  max_tiers : int;
      (** M2 recursive residual clustering (§4.4): cap on tier depth. A backstop
          — each emitting tier strictly shrinks the gap by the net-progress
          guard, so this is rarely reached. *)
  cost_byte_limit : int;
  ...
  max_selectors_per_pair : int;
      (** §3.2 cost containment: at most this many path selectors are enumerated
          per change pair when building anchored variants. *)
  ...
  emission_threshold : float;
      (** legacy change-density parameter for [collect_change_pairs_multi];
          retained for compatibility (multi-level emission no longer gates on it
          — a structural gate was re-tried 2026-07 and rejected: it starves the
          pure-rename lift, see the emission comment in {!Cs_propose}). *)
  dendrogram_bucket_cap : int;
      (** Maximum leaves entering one dendrogram (per root-sig bucket, two-sided
          and one-sided alike). A corpus with thousands of genuine sites
          (312-file daffodil: ~4500 def-level change pairs) makes the pairwise
          merge quadratic-to-cubic in the site count, yet discovery only needs
          *enough* instances — support and coverage come from the evaluation
          gate, which extends a rule to every file it fires in (§3.3), and sites
