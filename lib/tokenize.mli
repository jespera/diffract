(** Pattern tokenization: pattern body text -> [Stmatch.pattern_token list].
    ...
    Ellipsis handling: before parsing, each standalone [...] is rewritten to
    a unique placeholder identifier so the body parses; the placeholder
    leaves are recognized and converted back to [Siblings] wildcards. A
    [...] immediately followed by an identifier or [$] (a spread/rest
    operator such as [...args] or [...$rest]) is preserved as a literal
    [...] token, not treated as ellipsis.
    ...
    preamble's declarations are the sole authority. This treats [obj] and
    [$obj] identically; whichever the preamble declares is what's matched.
...
(** [tokenize ~ctx ~language ~single_metavars ~sequence_metavars body] returns
    the token list for [body]. Extras (comments, whitespace tokens) are
    skipped. *)
...
(** [tokenize_span ~ctx ~language ~single_metavars ~sequence_metavars ~lo ~hi
    text] tokenizes [text] but keeps only the leaves whose start byte is in
    the half-open range [\[lo, hi)]. No ellipsis preprocessing is performed,
    so byte offsets in [text] are stable; callers must therefore only use
    this on ellipsis-free text.

    This supports {b contextual tokenization}: a pattern fragment is spliced
    into a larger scaffold (so the parser assigns its leaves the node-types
    they would have in that context — e.g. an object key as
    [property_identifier] rather than the standalone [statement_identifier]),
    then the fragment's own leaves are extracted by their byte span. *)
