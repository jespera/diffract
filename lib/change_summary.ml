(** Change summary: infer the spatch-style rules behind a before/after changeset
    ...
    - {!Cs_propose} — change-pair extraction and the candidate channels
      (§3.1–3.2)
    - {!Cs_cluster} — anti-unification dendrogram, coarsening, one-sided
      clustering (§4.1)
    - {!Cs_evaluate} — the per-site safety gate that defines a rule's meaning
      (§3.3)
    - {!Cs_fusion} — conjunctive multi-section fusion of co-occurring changes
      (§4.2)
    - {!Cs_select} — one tier: propose → evaluate → greedy set-cover (§3.3)
    - {!Cs_tier} — the tiered loop, chain-effect accounting, residual emission
      (§4.4)
