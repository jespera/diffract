...
type ast_index = { by_type : (string, Tree.src Tree.t list) Hashtbl.t }
...
type match_mode =
  ...
  | Field
      (** Match by field name - extras in other fields ignored, ordered within
          fields *)
  | Partial
      (** Unordered subset matching - extras ignored, order doesn't matter *)

type child_correspondence = { pattern_index : int; source_index : int }
...
type pattern = {
  ...
  tree : Tree.pat Tree.tree;
      (** Parsed pattern tree (phantom-typed as pattern) *)
  ...
  on_var : string option;
      (** If set, match against the node bound to this var instead of traversing
      *)
  is_transform : bool;  (** Whether pattern has -/+ lines (semantic patch) *)
  replace_tree : Tree.pat Tree.tree option;
...
type match_result = {
  ...
  sequence_node_bindings : (string * Tree.src Tree.t list) list;
      (** Metavar name -> matched nodes for sequences *)
  correspondences : child_correspondence list;
      (** Pattern-to-source child index mapping *)
...
type nested_match_result = {
  ...
  bindings : (string * string) list;
      (** All bindings (inner + inherited from contexts) *)
  node_bindings : (string * Tree.src Tree.t) list;
  sequence_node_bindings : (string * Tree.src Tree.t list) list;
...
type nested_pattern = {
  patterns : pattern list;
      (** All sections; last is target, rest are contexts *)
}
