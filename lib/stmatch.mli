...
module Make (C : Cursor.S) : sig
  ...
  (** [match_at pattern cursor] tries to match [pattern] starting at [cursor]'s
      ...
      pre-existing bindings. Named wildcards encountered during this match that
      share a name with a seeded binding must bind to a structurally-equal
      subtree; otherwise the match fails. The returned bindings include the seed
      plus any new bindings from this call, in pattern order.
  ...
  (** [match_prefix pattern cursor] tries to match a prefix of [pattern] against
      ...
      [ignore_node_type] (default [false]): when [true], [Concrete] tokens match
      source leaves on text alone, ignoring node type. Used by field-mode search
      ...
      source-exhaustion is returned — a leading wildcard takes the whole child.
      ...
      [None] when [pattern] exhausts before the sub-tree (pattern was too short
      to cover the sub-tree) or when no match path succeeds.
      ...
      call [match_prefix] to consume one element's worth of pattern tokens. The
      remaining-pattern return value is fed into the next call when iterating
      over a sequence of source children.

      [initial_bindings] (default [[]]) seeds the binding state — the same
      semantics as in {!match_at}. The partial-mode driver uses this to thread
      bindings across element matches so that cross-element non-linearity is
      enforced.
  ...
  (** [match_set_at pattern cursor] is the low-level set-match primitive: it
      ...
      unordered, extras tolerated, separator-aware. It does NOT inspect
      leading/trailing anonymous delimiter runs; callers that want
      container-level partial matching should use {!match_partial_at}.
      ...
      - Source-derived separator stripping between successful element matches: a
        leading separator token (e.g. [","] for object literals, [";"] for
        record types) is stripped from the pattern. The separator is inferred
        from the bytes between the first two source named children; an empty
        inferred separator (e.g. JSX attributes) means no stripping.
      - Lenient: if the pattern omits the separator, the matcher proceeds
        without stripping.
      - Backtracking: ambiguous patterns explore alternative element-to-child
        assignments. Worst-case complexity is O(n!) for n source children with
        all-wildcard leading tokens; typical patterns with distinct leading
        concrete tokens per element are effectively linear because non-matching
        children fail [match_prefix] on the first token.
      - Cross-element non-linearity: accumulated bindings are threaded through
        each [match_prefix] call as [initial_bindings], so a named wildcard
        appearing in multiple elements must bind to structurally-equal subtrees
        across the match. Inconsistent assignments trigger backtracking or
        overall failure.
  ...
  (** [match_partial_at pattern cursor] is the partial-mode entry point for a
      ...
      produces as the container's anonymous delimiters.

      The algorithm splits the source container's non-extra children into:
      - A leading anonymous run ({!Cursor.S.leading_anonymous_leaves} — e.g.
        [['{']] for an object, [['<']] for a JSX element).
      - The named children ({!Cursor.S.named_children} — the elements).
      - A trailing anonymous run ({!Cursor.S.trailing_anonymous_leaves} — e.g.
        [['}']], [['/>']]).

      The pattern is split in lock-step with that structure: a front of
      [List.length leading] tokens strict-matched against the leading run, a
      back of [List.length trailing] tokens strict-matched against the trailing
      run, and a middle handed to {!match_set_at} for the element-level
      set-match against the named children.

      Structural self-filter: a candidate node must have {b both} a non-empty
      leading run and a non-empty trailing run — that's the property of a real
      bracketed container. Nodes without that shape (statements, positional
      declarations, non-bracketed expressions, ...) fail immediately. This rules
      out the corner case where a multi-token pattern would otherwise collapse
      into a single named child as a strict subtree match.

      Strict delimiter matching: front/back tokens must be [Concrete] and equal
      the source leaves on both text and node type. A wildcard in a delimiter
      slot fails.

      Returns [Some (cursor, bindings)] on success (cursor unchanged at the
      container). Returns [None] when the candidate node has no structural
      delimiters, when the delimiter runs don't strict-match, when the pattern
      is shorter than the combined delimiter run, or when {!match_set_at} fails
      on the middle.

      Test fixtures: cursor implementations that don't model the named/anonymous
      distinction (typically hand-built test trees) return [[]] from the
  ...
  (** [match_field_at pattern cursor] is the field-mode entry point. It aligns
      ...
      At each child the matcher either MATCHES it — consuming the prefix of the
      remaining pattern that exactly covers the child's subtree, via
      {!match_prefix} — or SKIPS it, leaving the pattern untouched. The choice
      is explored with backtracking, MATCH first. The pattern must be fully
      consumed; children remaining after the pattern is exhausted are skipped
      (tolerated extras).

      No per-language optionality data is consulted. Required fields are
      syntactically mandatory, so the pattern always contains them and they are
      matched in order; the only children a pattern can leave unmatched are the
      optional ones it omits (decorators, annotations, modifier groups, return
      types). MATCH-first guarantees a child the pattern addresses is matched
      fully — a pattern [f ()] does not match a source [f (a, b)] — while SKIP
      and backtracking give both "ignore what I didn't mention" and "match this
      element among several of its shape" (e.g. [@Get] skipping a leading
      [@Auth]).

      The walk is over {b all} non-extra children ({!Cursor.S.all_children}), so
      a declaration's structural keywords — anonymous leaves that are direct
      children (Kotlin [fun], Scala [def], a [function] keyword) — are seen and
      {b must} match; only {b named} children are skippable. Skipping an
      anonymous keyword the pattern omits (TS [async] / [abstract]) is the
      deferred Tier-3 case and so yields an honest no-match. A pattern's own
  ...
  (** [find_matches pattern source_cursor] returns matches of [pattern] in the
      ...
      - [false]: resume past the matched region — matches are non-overlapping
        ...
        transforms.
      - [true]: resume one node into the match, so matches nested inside a match
        are also reported; spans are de-duplicated so each distinct
        [(start_byte, end_byte)] appears once. Suited to search.

      [initial_bindings] (default [[]]) seeds the binding state for every
      [match_at] attempt during the walk. Named wildcards encountered during a
  ...
  (** [find_partial_matches pattern source_cursor] is the partial-mode outer
      ...
      bracket/punctuation tokens. The inner matcher reads the actual delimiter
      shape from each candidate source node, so the same pattern adapts
      naturally to whatever the grammar produced. Nodes that don't have the
      right structure (wrong leading/trailing anon, wrong element shape) fail
      cleanly — no explicit container classification is needed.
  ...
  (** Like {!find_field_matches}, but the pattern is computed per candidate node
      ...
      each candidate's surrounding source so context-sensitive node-types (e.g.
