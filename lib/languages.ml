...
let library_search_paths =
  ...
  in
  let env_path =
    match Sys.getenv_opt "DIFFRACT_GRAMMAR_PATH" with
    | Some p when p <> "" -> [ p ]
    | _ -> []
  in
...
let try_load_library lib_basename =
  ...
  let rec try_paths = function
    ...
    | path :: rest -> (
        let full_path =
          if path = "" then lib_name else Filename.concat path lib_name
        in
        try
...
let load_language lib_basename func_name =
  ...
  | None ->
      failwith
        (Printf.sprintf
           "Could not find library for %s. Make sure grammar libraries are \
            built (run grammars/build-grammars.sh) or install system packages."
           lib_basename)
  | Some lib -> (
      try
      ...
      with exn ->
        failwith
          (Printf.sprintf "Failed to load symbol %s: %s" func_name
...
let list_available () =
  ...
  let order = ref [] in
  List.iter
    (fun (name, (lib_basename, func_name)) ->
      ...
      | Some (func_name, canonical, aliases) ->
          if can_load_language lib_basename func_name then
            let label =
              ...
              | [] -> canonical
              | _ ->
                  Printf.sprintf "%s (aliases: %s)" canonical
