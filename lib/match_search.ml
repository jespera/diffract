...
(** Find all matches of a pattern within a specific subtree. Checks inherited
    bindings for conflicts and merges them with new bindings. *)
...
(** Find matches for a pattern, respecting the 'on $VAR' directive. If pattern
    has on_var, searches within the bound node(s). For single metavars, treats
    as a sequence of one node. Otherwise, traverses the subtree looking for
    matches. *)
let find_pattern_matches ~pattern ~inherited_bindings ~source
  ...
  | Some var_name ->
      ...
      let target_nodes =
        ...
        | None -> (
            match
              List.assoc_opt var_name inherited_bindings.sequence_node_bindings
            with
            | Some nodes -> nodes
            | None ->
                failwith
                  (Printf.sprintf
                     "on %s: variable not bound in previous pattern" var_name))
      in
      (* Search within each target node *)
      List.concat_map
        (fun target_node ->
          find_matches_in_subtree ~pattern ~inherited_bindings ~source
...
let find_matches ~ctx ~language ~pattern_text ~source_text =
  ...
  find_matches_in_subtree ~pattern
    ~inherited_bindings:Match_engine.empty_bindings ~source:source_tree.source
...
(** Recursively find nested matches. Process patterns left-to-right. For each
    context pattern, find matches, then recurse into each match's subtree with
    remaining patterns. Respects 'on $VAR' directive for direct matching against
    bound nodes. *)
let rec find_nested_matches_impl ~source ~inherited_bindings
  ...
  | [ target ] ->
      ...
      let matches =
        find_pattern_matches ~pattern:target ~inherited_bindings ~source
          ~root_node
      in
      List.map
        (fun (m : match_result) ->
  ...
  | ctx :: rest ->
      ...
      let ctx_matches =
        find_pattern_matches ~pattern:ctx ~inherited_bindings ~source ~root_node
      in
      List.concat_map
        (fun (ctx_match : match_result) ->
          let ctx_record =
            {
...
(** Find nested matches using a pattern with multiple sections. Auto-detects:
    single section behaves like find_matches, multiple sections enable
    nested/scoped matching. Supports 'on $VAR' for direct matching. *)
let find_nested_matches ~ctx ~language ~pattern_text ~source_text =
  let matches, _ =
    find_nested_matches_internal ~ctx ~language ~pattern_text ~source_text
  in
  matches

(** Find nested matches and return parse error information. Returns both matches
    and the count of parse errors (ERROR nodes) in the source. *)
...
(** Find matches using a pre-built index. Falls back to full traversal if
    pattern root is a metavar. *)
let find_matches_with_index ~index ~pattern ~source
    ~(source_root : Tree.src Tree.t) =
  let pattern_node = Match_engine.get_pattern_content pattern in
  (* Check if pattern root is a metavar placeholder *)
  match
    Match_engine.is_placeholder pattern.substitutions pattern.source
      pattern_node
  with
  | Some _ ->
  ...
  | None ->
      ...
      let candidates =
        Hashtbl.find_opt index.by_type pattern_type |> Option.value ~default:[]
      in
      List.filter_map
        (fun source_node ->
          match
            Match_engine.match_node ~pattern ~pattern_source:pattern.source
              ~source ~substitutions:pattern.substitutions pattern_node
              source_node
          with
          | Some mb ->
              Some
...
(** Match multiple patterns against source, building index once. Returns list of
...
(** Format a nested match result for display. Shows each context level with
    indentation and all bindings. *)
let format_nested_match source_text result =
  ...
  let format_bindings indent bindings =
    let unique =
      List.sort_uniq (fun (v1, _) (v2, _) -> String.compare v1 v2) bindings
    in
    List.iter
      (fun (var, value) ->
  ...
  List.iteri
    (fun i ctx ->
      ...
      Buffer.add_string buf
        (Printf.sprintf "context[%d] line %d: %s\n" i line preview);
