(** Benchmarks for index-based pattern matching
    ...
    particularly for multi-pattern scenarios where indexing provides significant
    benefits. *)
...
let generate_source ~num_logs ~num_errors ~num_classes =
  let logs =
    List.init num_logs (fun i -> Printf.sprintf "console.log(\"log%d\");" i)
    |> String.concat "\n"
  in
  let errors =
    List.init num_errors (fun i -> Printf.sprintf "console.error(\"err%d\");" i)
    |> String.concat "\n"
  in
  let classes =
    List.init num_classes (fun i ->
...
let () =
  ...
  List.iter
    (fun (num_logs, num_errors, num_classes) ->
      let source = generate_source ~num_logs ~num_errors ~num_classes in
      let label =
        Printf.sprintf "%d nodes" (num_logs + num_errors + (num_classes * 3))
      in

      let traversal_time =
        time_median 3 (fun () ->
            bench_traversal_multi ~language:"typescript" ~patterns
              ~source_text:source)
      in

      let indexed_time =
        time_median 3 (fun () ->
            bench_indexed_multi ~language:"typescript" ~patterns
              ~source_text:source)
      in
      ...
      in
      let speedup =
        if indexed_time > 0.0 then traversal_time /. indexed_time else 0.0
      in

      Printf.printf "%-18s %10.2f ms %10.2f ms %9.2fx %8d\n" label
        (traversal_time *. 1000.0) (indexed_time *. 1000.0) speedup
        (List.length matches);
  ...
  for n = 1 to 3 do
    ...
    let traversal_time =
      time_median 3 (fun () ->
          bench_traversal_multi ~language:"typescript" ~patterns:pattern_subset
            ~source_text:source)
    in

    let indexed_time =
      time_median 3 (fun () ->
          bench_indexed_multi ~language:"typescript" ~patterns:pattern_subset
            ~source_text:source)
    in
...
const $name = $val|}
  ...
  Printf.printf "%-20s %12s %12s %10s %8s\n" "Pattern Type" "Traversal"
    "Indexed" "Speedup" "Matches";
  Printf.printf "%s\n" (String.make 68 '-');

  List.iter
    (fun (label, pattern_text) ->
      let traversal_time =
        time_median 3 (fun () ->
            Match.find_matches ~ctx ~language:"typescript" ~pattern_text
              ~source_text:selective_source)
      in
  ...
  List.iter
    (fun pattern_text ->
      let pattern =
        Match.parse_pattern ~ctx ~language:"typescript" pattern_text
      in
      let query_time =
        time_median 5 (fun () ->
      ...
      Printf.printf "%-35s %8.3f ms %8d\n" pattern_preview
        (query_time *. 1000.0) (List.length matches);
      Gc.full_major ())
    patterns;
  ...
  Printf.printf
    "Use build_index + find_matches_with_index for custom workflows.\n"
