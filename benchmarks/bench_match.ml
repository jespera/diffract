    ...
    (Staged.stage (fun () ->
         ignore
           (Diffract.Match.find_matches ~ctx ~language:"typescript"
              ~pattern_text:simple_pattern ~source_text:simple_source);
         maybe_gc ()))
...
let bench_sequence n =
  ...
  Test.make
    ~name:(Printf.sprintf "sequence_%02d_children" n)
    (Staged.stage (fun () ->
         ignore
           (Diffract.Match.find_matches ~ctx ~language:"typescript"
...
let bench_nested_sequence n =
  let source = generate_class_source n in
  Test.make
    ~name:(Printf.sprintf "nested_seq_%02d_children" n)
    (Staged.stage (fun () ->
         ignore
...
let save_results_json analyzed_results instances =
  ...
  List.iter
    (fun instance ->
      let merged =
        Analyze.merge
          Analyze.(
            ols ~bootstrap:0 ~r_square:true ~predictors:[| Measure.run |])
          [ instance ] analyzed_results
      in
      Hashtbl.iter
        (fun _measure_label results_tbl ->
          Hashtbl.iter
            (fun name ols_result ->
              match Analyze.OLS.estimates ols_result with
              | Some [ v ] -> bench_data := (name, v) :: !bench_data
              | _ -> ())
            results_tbl)
        merged)
    instances;
  ...
  let benchmarks_json =
    List.map
      (fun (name, mean_ns) ->
        `Assoc [ ("name", `String name); ("mean_ns", `Float mean_ns) ])
      !bench_data
  in
  ...
  let filename =
    Printf.sprintf "benchmarks/results/%d-%s.json" timestamp_int git_commit
  in

  (* Ensure directory exists *)
  (try Unix.mkdir "benchmarks/results" 0o755
  ...
  List.iter
    (fun analyzed ->
      Hashtbl.iter
        (fun name ols_result ->
          match Analyze.OLS.estimates ols_result with
          | Some [ v ] -> Printf.printf "%-35s %15.2f\n" name v
          | _ -> ())
        analyzed)
    analyzed_results;
