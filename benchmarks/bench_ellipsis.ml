(** Benchmarks for ellipsis (...) sequence matching
    ...
    This benchmark measures performance of patterns with ... (ellipsis) to
    identify optimization opportunities using FIRST-set lookahead.

    Key insight: pattern `... console.log($X) ...` currently tries every split
    point. With FIRST-set optimization, we only try positions where the anchor
...
let generate_function_body ~n ~target_call ~target_positions =
  let stmts =
    List.init n (fun i ->
        if List.mem i target_positions then Printf.sprintf "    %s;" target_call
...
let pattern_two_anchors target1 target2 =
  Printf.sprintf
    {|@@
match: strict
@@
...
let () =
  ...
  List.iter
    (fun n ->
      ...
      let source =
        generate_source ~n ~target_call:target ~target_positions:[ n / 2 ]
      in
      ...
      Printf.printf "%-12d %12.3f %12d %10.1f\n" n (time *. 1000.0)
        (List.length matches) us_per_stmt;
  ...
  Printf.printf
    "If us/stmt grows with n: O(n²) or worse (optimization needed)\n";
  ...
  List.iter
    (fun num_matches ->
      ...
      let positions =
        List.init num_matches (fun i ->
            (i * n / num_matches) + (n / num_matches / 2))
      in
      let source =
        generate_source ~n ~target_call:target ~target_positions:positions
      in
      ...
      let ms_per_match =
        if List.length matches > 0 then
          time *. 1000.0 /. float_of_int (List.length matches)
        else 0.0
      in

      Printf.printf "%-20d %12.3f %12d %10.3f\n" num_matches (time *. 1000.0)
        (List.length matches) ms_per_match;
  ...
  List.iter
    (fun n ->
      ...
      let time =
        time_median 3 (fun () ->
            Match.find_matches ~ctx ~language:"typescript"
              ~pattern_text:pattern2 ~source_text:source)
      in

      let matches =
        Match.find_matches ~ctx ~language:"typescript" ~pattern_text:pattern2
          ~source_text:source
      in
  ...
  Printf.printf
    "If time roughly 8x when n doubles: O(n³) - needs optimization\n";
  ...
  let source =
    generate_source ~n:100 ~target_call:target ~target_positions:[ 50 ]
  in
  ...
  List.iter
    (fun (label, pat) ->
      let time =
        time_median 5 (fun () ->
            Match.find_matches ~ctx ~language:"typescript" ~pattern_text:pat
              ~source_text:source)
      in
  ...
  Printf.printf
    "This forces trying all positions before concluding no match\n\n";
  ...
  List.iter
    (fun n ->
      ...
      let time =
        time_median 3 (fun () ->
            Match.find_matches ~ctx ~language:"typescript"
              ~pattern_text:pattern_no_match ~source_text:source)
      in
...
function test() {
  ...
  let test_configs =
    [ (20, 10); (* 20 stmts, 10 are call() *) (40, 20); (80, 40); (160, 80) ]
  in
  ...
  List.iter
    (fun (n, num_calls) ->
      ...
      let time =
        time_median 3 (fun () ->
            Match.find_matches ~ctx ~language:"typescript"
              ~pattern_text:pattern_many_false ~source_text:source)
      in
  ...
  Printf.printf
    "  - For 'call()', only try positions with call_expression nodes\n";
  ...
  Printf.printf
    "Source: mix of let declarations, if statements, and few calls\n";
  Printf.printf
    "Index optimization: only try positions with call_expression\n\n";

  Printf.printf "%-12s %-12s %12s %10s\n" "Statements" "# calls" "Time (ms)"
    "us/stmt";
  Printf.printf "%s\n" (String.make 50 '-');
...
function test() {
  ...
  List.iter
    (fun (n, num_calls) ->
      ...
      let body =
        List.init n (fun i ->
            let call_positions = [ n / 4; n / 2; 3 * n / 4; n - 1 ] in
            (* 4 calls at specific positions *)
            if List.mem i call_positions && i < num_calls then
              ...
              Printf.sprintf "    if (x%d) { return; }" (i / 2)
              (* if statement *)
            else Printf.sprintf "    const y%d = x%d + 1;" i (i / 2)
            (* another declaration *))
        |> String.concat "\n"
      in
      let source = Printf.sprintf "function test() {\n%s\n}" body in
      ...
      let time =
        time_median 3 (fun () ->
            Match.find_matches ~ctx ~language:"typescript"
              ~pattern_text:pattern_mixed ~source_text:source)
      in
  ...
  Printf.printf
    ...
    (100 * 4 / 20);
  Printf.printf
    "Expected: O(n) scaling since call count grows linearly with n\n";
  ...
  Printf.printf
    "  - For '... X ...', only try positions where X's type appears\n";
  Printf.printf
    "  - Complexity: O(n) for realistic code with mixed statement types\n\n";
  ...
  Printf.printf
    "  - Worst case (all same type): falls back to trying all positions\n"
