...
let read_result_file filename =
  ...
  let benchmarks = json |> member "benchmarks" |> to_list |> List.map (fun b ->
...
let pct_change old_val new_val =
  if old_val = 0.0 then 0.0
  else ((new_val -. old_val) /. old_val) *. 100.0
...
let color_pct pct =
  ...
  if pct > 5.0 then red s
  else if pct < -5.0 then green s
  else yellow s
...
let print_comparison (ts1, commit1, bench1) (ts2, commit2, bench2) =
  ...
  let bench1_map = List.fold_left (fun acc (name, v) ->
    (name, v) :: acc
  ) [] bench1 in

  List.iter (fun (name, new_val) ->
...
let print_history () =
  ...
  List.iter (fun (_, commit, _) ->
    Printf.printf " %12s" commit
  ) results;
  Printf.printf "\n";
  ...
  List.iter (fun name ->
    Printf.printf "%-25s" (if String.length name > 24 then String.sub name 0 24 else name);
    List.iter (fun (_, _, benchmarks) ->
...
let generate_gnuplot_data () =
  ...
  List.iteri (fun i name ->
    Printf.fprintf oc "# Column %d: %s\n" (i + 2) name
  ) all_names;
  Printf.fprintf oc "\n";

  (* Data rows *)
  List.iter (fun (ts, _, benchmarks) ->
    Printf.fprintf oc "%.0f" ts;
    List.iter (fun name ->
      match List.assoc_opt name benchmarks with
      | Some v -> Printf.fprintf oc " %.2f" (v /. 1000.0)  (* Convert to microseconds *)
  ...
  List.iteri (fun i name ->
    ...
    Printf.fprintf oc "'%s' using 1:%d with linespoints title '%s'"
      data_file (i + 2) name
  ) all_names;
  Printf.fprintf oc "\n";
  close_out oc;
  ...
  Printf.printf "For PNG output, change 'set terminal dumb' to 'set terminal png' and add 'set output \"graph.png\"'\n"
...
let print_mini_history () =
  ...
  Printf.printf "\n%s\n\n" (bold (Printf.sprintf "Benchmark Trends (last %d runs)" (List.length results)));
  ...
  List.iter (fun name ->
    let values = List.filter_map (fun (_, _, benchmarks) ->
      List.assoc_opt name benchmarks
    ) results in
...
let () =
  ...
  | [] ->
    ...
    if List.length files < 2 then begin
      ...
      Printf.printf "Run benchmarks with: dune exec benchmarks/bench_match.exe\n";
      exit 1
    end;
    let last_two = files |> List.rev |> (fun l -> match l with
      | a :: b :: _ -> [b; a]
      | _ -> l
    ) in
  ...
  | _ ->
    Printf.printf "Usage:\n";
    Printf.printf "  bench_compare                      Compare last 2 results\n";
    Printf.printf "  bench_compare file1.json file2.json  Compare specific files\n";
    Printf.printf "  bench_compare --history            Show history table\n";
    Printf.printf "  bench_compare --trends             Show trends with sparklines\n";
    Printf.printf "  bench_compare --gnuplot            Generate gnuplot data files\n";
    exit 1
